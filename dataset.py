...
from utils.general import get_rally_dirs
...
class Shuttlecock_Trajectory_Dataset(Dataset):
    ...
    def _gen_rally_img_congif_file(self, file_name):
        ...
        for rally_i, rally_dir in tqdm(self.rally_dict['i2p'].items()):
            w, h = Image.open(os.path.join(rally_dir, f'0.{self.img_format}')).size
    ...
    def _gen_input_from_rally_dir(self, rally_dir):
        ...
        if self.data_mode == 'heatmap':
            ...
            f_file = np.array([os.path.join(rally_dir, f'{f_id}.{self.img_format}') for f_id in label_df['Frame']])
            ...
            f_file = np.array([os.path.join(rally_dir, f'{f_id}.{self.img_format}') for f_id in pred_df['Frame']])
